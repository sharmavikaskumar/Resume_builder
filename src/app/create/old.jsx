...
const INITIAL = {
  ...
  education:      [{ id: 1, institution: "", degree: "", startYear: "", endYear: "", gpa: "" }],
  experience:     [{ id: 1, company: "", role: "", startDate: "", endDate: "", description: "" }],
  skills:         [],
  projects:       [{ id: 1, name: "", techStack: "", link: "", description: "" }],
  certifications: [{ id: 1, name: "", issuer: "", issueDate: "", expiryDate: "" }],
...
function EducationForm({ data, onChange }) {
  const update = (id, f) => (e) => onChange(data.map((d) => d.id === id ? { ...d, [f]: e.target.value } : d));
  const add    = () => onChange([...data, { id: Date.now(), institution: "", degree: "", startYear: "", endYear: "", gpa: "" }]);
  const remove = (id) => onChange(data.filter((d) => d.id !== id));
  return (
    <div>
      <SectionTitle title="Education" description="Add your degrees, diplomas, or relevant coursework." />
      {data.map((edu, i) => (
        <Card key={edu.id}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            ...
            <Input label="Degree"      placeholder="B.Sc. Computer Science" value={edu.degree}      onChange={update(edu.id, "degree")} />
          </div>
          <div className="grid grid-cols-3 gap-5">
            <Input label="Start year" placeholder="2018"      value={edu.startYear} onChange={update(edu.id, "startYear")} />
            <Input label="End year"   placeholder="2022"      value={edu.endYear}   onChange={update(edu.id, "endYear")} />
            <Input label="GPA" optional placeholder="3.9/4.0" value={edu.gpa}       onChange={update(edu.id, "gpa")} />
          </div>
        </Card>
      ))}
...
function ExperienceForm({ data, onChange }) {
  const update = (id, f) => (e) => onChange(data.map((d) => d.id === id ? { ...d, [f]: e.target.value } : d));
  const add    = () => onChange([...data, { id: Date.now(), company: "", role: "", startDate: "", endDate: "", description: "" }]);
  const remove = (id) => onChange(data.filter((d) => d.id !== id));
  return (
    <div>
      <SectionTitle title="Work experience" description="List your roles from most recent to oldest." />
      {data.map((exp, i) => (
        <Card key={exp.id}>
          <CardHeader index={i} onRemove={() => remove(exp.id)} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            <Input label="Company"    placeholder="Google"            value={exp.company} onChange={update(exp.id, "company")} />
            <Input label="Role/Title" placeholder="Software Engineer" value={exp.role}    onChange={update(exp.id, "role")} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            <Input label="Start date" placeholder="Jun 2022" value={exp.startDate} onChange={update(exp.id, "startDate")} />
            <Input label="End date"   placeholder="Present"  value={exp.endDate}   onChange={update(exp.id, "endDate")} />
          </div>
          <Textarea
            label="Responsibilities & achievements"
            placeholder={"• Built X, resulting in Y\n• Led a team of N engineers to deliver Z"}
...
function ProjectsForm({ data, onChange }) {
  const update = (id, f) => (e) => onChange(data.map((d) => d.id === id ? { ...d, [f]: e.target.value } : d));
  const add    = () => onChange([...data, { id: Date.now(), name: "", techStack: "", link: "", description: "" }]);
  const remove = (id) => onChange(data.filter((d) => d.id !== id));
  return (
    <div>
      <SectionTitle title="Projects" description="Highlight personal or professional projects that showcase your skills." />
      {data.map((p, i) => (
        <Card key={p.id}>
          <CardHeader index={i} onRemove={() => remove(p.id)} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            <Input label="Project name" placeholder="ResumeForge"                   value={p.name}      onChange={update(p.id, "name")} />
            <Input label="Tech stack"   placeholder="Next.js, TypeScript, Tailwind" value={p.techStack} onChange={update(p.id, "techStack")} />
          </div>
          <div className="mb-5">
...
function CertsForm({ data, onChange }) {
  const update = (id, f) => (e) => onChange(data.map((d) => d.id === id ? { ...d, [f]: e.target.value } : d));
  const add    = () => onChange([...data, { id: Date.now(), name: "", issuer: "", issueDate: "", expiryDate: "" }]);
  const remove = (id) => onChange(data.filter((d) => d.id !== id));
  return (
    <div>
      <SectionTitle title="Certifications" description="Include relevant professional certifications or licenses." />
      {data.map((c, i) => (
        <Card key={c.id}>
          <CardHeader index={i} onRemove={() => remove(c.id)} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
            <Input label="Certification name"   placeholder="AWS Solutions Architect" value={c.name}   onChange={update(c.id, "name")} />
            <Input label="Issuing organization" placeholder="Amazon Web Services"     value={c.issuer} onChange={update(c.id, "issuer")} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Input label="Issue date"           placeholder="Mar 2023" value={c.issueDate}  onChange={update(c.id, "issueDate")} />
            <Input label="Expiry date" optional placeholder="Mar 2026" value={c.expiryDate} onChange={update(c.id, "expiryDate")} />
          </div>
        </Card>
      ))}
...
function AchievementsForm({ data, onChange }) {
  const update = (id, f) => (e) => onChange(data.map((d) => d.id === id ? { ...d, [f]: e.target.value } : d));
  const add    = () => onChange([...data, { id: Date.now(), title: "", year: "", description: "" }]);
  const remove = (id) => onChange(data.filter((d) => d.id !== id));
  return (
    <div>
      <SectionTitle title="Achievements" description="Awards, honors, publications, or other notable accomplishments." />
      {data.map((a, i) => (
        <Card key={a.id}>
...
function ContactForm({ data, onChange }) {
  ...
  return (
    <div>
      <SectionTitle title="Contact information" description="These appear in your resume header alongside your name." />
      <Card>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
          <Input label="Email" type="email" placeholder="john@email.com"    value={data.email} onChange={set("email")} />
          <Input label="Phone" type="tel"   placeholder="+1 (555) 000-0000" value={data.phone} onChange={set("phone")} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
          <Input label="LinkedIn URL" placeholder="linkedin.com/in/johndoe" value={data.linkedin} onChange={set("linkedin")} />
          <Input label="GitHub URL"   placeholder="github.com/johndoe"      value={data.github}   onChange={set("github")} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <Input label="Portfolio" optional placeholder="johndoe.dev"       value={data.portfolio} onChange={set("portfolio")} />
          <Input label="Location"           placeholder="San Francisco, CA" value={data.location}  onChange={set("location")} />
        </div>
      </Card>
    </div>
  );
...
export default function CreatePage() {
  ...
  const renderSection = () => {
    ...
    if (id === "personal")       return <PersonalForm      data={data.personal}       onChange={set("personal")} />;
    if (id === "education")      return <EducationForm     data={data.education}      onChange={set("education")} />;
    if (id === "experience")     return <ExperienceForm    data={data.experience}     onChange={set("experience")} />;
    if (id === "skills")         return <SkillsForm        data={data.skills}         onChange={set("skills")} />;
    if (id === "projects")       return <ProjectsForm      data={data.projects}       onChange={set("projects")} />;
    if (id === "certifications") return <CertsForm         data={data.certifications} onChange={set("certifications")} />;
        ...
        <header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 md:px-8 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 text-sm text-gray-400">
              ...
              <span className="font-semibold text-black">{SECTIONS[current].label}</span>
            </div>
